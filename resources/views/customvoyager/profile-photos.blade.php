...
@section('content')
    ...
    <div class="page-content container-fluid">
        ...
        <form action="{{action('CustomVoyagerController@changeProfilePhotos')}}" method="POST" class="upload_field">
            {{ csrf_field() }}
            <h4>{{$item->title}}</h4>
                <select name="photo">
                    <option value="" {{$item->img == '' || $item->img == null ? 'selected' : ''}}></option>
                    @forelse ($availablePhotos as $photo)
                        <option value="{{$photo}}" {{substr($item->img, 17) == $photo ? 'selected' : ''}}>{{$photo}}</option>
                    @empty
                    @endforelse
                </select>
                <input type="hidden" name="id" value="{{$item->id}}">
                <input type="submit" value="uložit změnu">
                <a href="https://kralsyru.cz/parovani-syry/{{$item->link}}" target="_blank">https://kralsyru.cz/parovani-syry/{{$item->link}}</a>
                <div class="clear"></div>
        </form>
        @empty
        @endforelse
        ...
        <form action="{{action('CustomVoyagerController@saveProfilePhotos')}}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
        ...
        <div class="gallery">
                ...
                @forelse ($availablePhotos as $item)
                    <div class="img">
                        <a href="/storage/profile/{{$item}}" target="_blank"><img src="/storage/profile/{{$item}}"></a>
                        <div class="name">{{$item}}</div>
                        <div class="delete"><a href="{{action('CustomVoyagerController@deleteProfilePhotos')}}?file={{$item}}">smazat</a></div>
                    </div>
